...
# Takes model output and true labels
...
def run_epoch(model,loader,criterion,optimizer,device,train: bool,epoch: int,epochs: int,) -> Tuple[float, float]:
    ...
    """
    One pass over loader with a live progress bar.
    ...
    for xb, yb in bar:
        ...
        # Live running averages in the bar
        bar.set_postfix(
...
# Save learning-curve PNG (up to this epoch) and write history.json.
...
def train_model(model, loaders, epochs, lr, device, patience=3):
    ...
    """
    Train the model and:
      - show tqdm progress bars for train/val each epoch
      - log metrics to TensorBoard (runs/recycle)
      - save best weights to models/resnet18_best.pt
      - save a curve PNG to reports/ after every epoch
    """
    ...
    writer = SummaryWriter("runs/recycle")  # view with: tensorboard --logdir runs
